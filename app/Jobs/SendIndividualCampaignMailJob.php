...
class SendIndividualCampaignMailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (!$camp_data) {
            Log::error("Campagne introuvable ou non en cours d'envoi : ID {$this->campaign}");
            return;
        }

        // Récupérer les informations du serveur mail
        $mail_server = Email_service::where("mail_username", $camp_data->from_email)->first();

        if (!$mail_server) {
            Log::error("Aucun serveur mail trouvé pour l'email : {$camp_data->from_email}");
        ...
        if ($contacts->isEmpty()) {
            Log::info("Aucun contact trouvé pour la campagne : ID {$this->campaign}");
            Campaign::where("id", $this->campaign)->update(["status" => "sent", "updated_at" => now()]);
            return;
        }
        ...
        $contacts->chunk($batch_size)->each(function ($batch) use ($camp_data, $mail_server, $max_per_hour, $max_per_day, $max_per_sec) {
            foreach ($batch as $contact) {
                ...
                if ($sent_last_5_seconds >= $max_per_sec) {
                    ...
                    sleep(5); // Attendre 5 secondes avant de continuer
                    continue;
                }
